...
Widget pageFavourite({BuildContext context, List<ProductData> list4,Key key,ScrollController controller}) {
  Size size = MediaQuery.of(context).size;

  return Container(
      height: size.height / 2.133333333333333,
      child: ListView.builder(
        padding: EdgeInsets.only(left: 25, right: 6),controller: controller,
        physics: BouncingScrollPhysics(),key: key,
        scrollDirection: Axis.horizontal,
        itemBuilder: (context, index) {
          final List<ProductData> list1 = list4.toList();
          list1.sort(
              (a, b) => int.parse(a.yeuThich).compareTo(int.parse(b.yeuThich)));
          final List<ProductData> list2=list1.reversed.toList();
...
Widget _priceProduct({String pricee}) {
  return Text(
    "Giá bìa: " + "₫${price.format(int.parse(pricee))}",
    style: TextStyle(
      fontSize: 12,
